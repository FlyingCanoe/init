...
fn switch_stdio(stdio: &str) -> Result<()> {
    ...
    syscall::dup2(stdin.as_raw_fd() as usize, 0, &[])
        .map_err(|err| Error::from_raw_os_error(err.errno))?;
    syscall::dup2(stdout.as_raw_fd() as usize, 1, &[])
        .map_err(|err| Error::from_raw_os_error(err.errno))?;
    syscall::dup2(stderr.as_raw_fd() as usize, 2, &[])
        .map_err(|err| Error::from_raw_os_error(err.errno))?;
...
impl PathExt for Path {
    ...
    fn scheme(&self) -> Option<PathBuf> {
        let path = self.as_os_str().to_string_lossy();

        path.find(':').map(|i| path[..i + 1].into())
    }
}
...
pub fn main() {
    ...
    if let Ok(_) = fs::metadata("initfs:/etc/init.rc") {
        ...
        let initfs_services = Service::from_dir(INITFS_SERVICE_DIR).unwrap_or_else(|err| {
            error!(
                "failed to parse service directory '{}': {}",
                INITFS_SERVICE_DIR, err
            );
            vec![]
        });
        ...
        env::set_current_dir("file:").unwrap_or_else(|err| {
            error!("failed to set cwd: {}", err);
        });
        // */
        let fs_services = Service::from_dir(FS_SERVICE_DIR).unwrap_or_else(|err| {
            error!(
                "failed to parse service directory '{}': {}",
                FS_SERVICE_DIR, err
            );
