...
impl ServiceGraph {
    ...
    pub fn push_services(&self, mut services: Vec<Service>) {
        let mut graph = self.graph.write().expect("service graph mutex poisoned");
    ...
    pub fn start_service(&self, index: Index) -> Result<(), Error> {
        let graph = self.graph.read().expect("service graph lock poisoned");
    ...
    fn start_service_with_graph(
        &self,
        graph: &DepGraph<Service>,
        index: Index,
    ) -> Result<(), Error> {
        let service = graph.get(index).ok_or(err_msg("service not found"))?;
