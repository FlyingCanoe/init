                ...
                if !seen.contains(&index)
                    && (node.dependencies.is_empty()
                        || node
                            .dependencies
                            .iter()
                            .all(|index| resolved.contains(index)))
                {
                    seen.insert(index);
                    resolved.push(index);
                }
    ...
    pub fn grouped_resolve(&self) -> Vec<Vec<Index>> {
        ...
        while seen.len() < arena_len {
            for (index, node) in self.graph.iter() {
                if !seen.contains(&index)
                    && (node.dependencies.is_empty()
                        || node.dependencies.iter().all(|index| {
                            if group_count != 0 {
                                groups[0..group_count - 1]
                                    .iter()
                                    .flatten()
                                    .any(|i| i == index)
                            } else {
